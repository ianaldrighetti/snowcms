...
{
  // Listbox?
  if(id.substr(-6) != '_other' && _.G(id).value != '-other-')
    _.G(id + '_other').value = _.G(id).value;
  // 'Other' textbox?
  else
  {
    id = id.substr(0, id.length - 6);
    _.G(id).selectedIndex = _.G(id).options.length - 1;
    for(option in _.G(id).options)
    {
      if(_.G(id + '_other').value == _.G(id).options[option].value)
        _.G(id).selectedIndex = option;
    }
  }
}