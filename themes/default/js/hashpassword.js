...
function hashPassword(handle)
{
  if(handle['passwrd'].value.length > 0)
  {
    var passwrd = SHA1.hash(SHA1.hash(handle['passwrd'].value) + loginHash);
    handle['hashed_passwrd'].value = passwrd;
    handle['passwrd'].value = '';
  }
}
