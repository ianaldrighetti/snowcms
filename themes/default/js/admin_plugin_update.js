...
{
  s.ajaxCallback(base_url + '/index.php?action=admin&sa=ajax&id=plugins_update&step=1&sid=' + session_id, function(response)
    {
      ...
      {
        s.id('download_status').innerHTML = response['message'];
        check_plugin_status();
      }
    }, 'dependency_name=' + s.encode(dependency_name) + '&version=' + s.encode(version));


  var h3 = document.createElement('h3');
  h3.innerHTML = l['downloading update'];
...
{
  s.ajaxCallback(base_url + '/index.php?action=admin&sa=ajax&id=plugins_update&step=2&sid=' + session_id, function(response)
    {
      var response = s.json(response);
...
{
  s.ajaxCallback(base_url + '/index.php?action=admin&sa=ajax&id=plugins_update&step=3&sid=' + session_id, function(response)
    {
      var response = s.json(response);
...
function cancel_install(element)
{
  ...
  s.ajaxCallback(base_url + '/index.php?action=admin&sa=ajax&id=plugins_update&step=cancel&sid=' + session_id, function(response)
    {
...
{
  s.ajaxCallback(base_url + '/index.php?action=admin&sa=ajax&id=plugins_update&step=4&sid=' + session_id, function(response)
    {
      var response = s.json(response);
