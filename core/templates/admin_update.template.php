...
if(!empty(api()->context['start_update_failure']))
{
	echo '
	<div class="error-message">
		<p>', l('The update process could not start because the base directory was not writable.'), '</p>
	</div>';
}
...
echo '
	...
	<p class="right">', api()->context['update_available'] ? '<a href="'. baseurl. '/index.php?action=admin&amp;sa=update&amp;start&amp;sid='. member()->session_id(). '" title="'. l('Apply update v%s', api()->context['latest_version']). '">'. l('Apply Update'). '</a> | ' : '', ' <a href="', baseurl, '/index.php?action=admin&amp;sa=update&amp;check" title="', l('Check for Updates'), '">', l('Check for Updates'), '</a></p>';
?>